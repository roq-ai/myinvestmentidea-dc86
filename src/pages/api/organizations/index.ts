...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.investment_option?.length > 0) {
      const create_investment_option = body.investment_option;
      body.investment_option = {
        create: create_investment_option,
      };
    } else {
      delete body.investment_option;
    }
